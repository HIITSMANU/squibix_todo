...
const Todo = () => {
    ...
    const filteredTodos = todos.filter(item => {
        const matchesSearchQuery = item.text.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    return (
        <div className={`w-full h-full overflow-y-auto mb-[10%] p-4 max-w-lg mx-auto flex flex-col shadow-md border-2 ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
            {loading ? (
                <>
                    <InputLoading />
                </>
                ...
                <>
                    <div className="flex flex-col sm:flex-row my-2 gap-4 justify-between">
                        <div className="flex flex-col flex-1">
                            ...
                            <div className='flex flex-row flex-1 items-center'>
                                ...
                                <button title='Add Your Todo' onClick={add} className='w-10 h-10 flex items-center justify-center bg-orange-600 text-white rounded hover:bg-orange-700 cursor-progress -ml-10'>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
                        <div className="flex flex-1 flex-col">
                            <p className='text-gray-400 text-[14px] italic'><span className='text-red-600'>*</span>Search Todos</p>
                            <input 
                                type="text" 
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                placeholder="Search todos..."
                                className={`flex-1 px-4 py-2 rounded border ${theme === 'dark' ? 'border-gray-600 bg-gray-700 text-white' : 'border-orange-400'} focus:outline-none focus:ring-2 focus:ring-orange-500`}
                            />
                        </div>
                    </div>
                </>
